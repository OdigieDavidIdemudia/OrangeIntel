...
public class ThreatIngestionService
{
    ...
    private async Task<(int, string)> IngestAlienVaultOtxAsync()
    {
        try
        {
            ...
            _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("TealHunt/1.0");
    ...
    public async Task<int> BroadcastRecentAlertsAsync()
    {
        ...
        // 2. Format Message
        var title = "📢 TealHunt | Critical Alerts Broadcast";
        ...
        // --- EMAIL ALERTS ---
        var emailProvider = _notificationProviders.FirstOrDefault(p => p.Name == "Email");
        if (emailProvider != null)
        {
            var emailBody = @"Dear Team,
Please be informed of an ongoing cyber espionage campaign targeting public Wi-Fi infrastructure at hotels, conference centers, and event venues. Threat actors are compromising network gateway devices and performing DNS poisoning attacks to redirect users attempting to access Microsoft 365 services to attacker-controlled phishing infrastructure.

Key Highlights
- Connecting to a compromised public Wi-Fi network alone may expose users to credential theft, even without clicking malicious links or opening attachments.
- Attackers redirect legitimate Microsoft 365 login traffic to convincing lookalike pages designed to harvest credentials and OAuth tokens. 
- The campaign reportedly abuses Microsoft Device Code authentication flows to bypass traditional MFA protections.
- Traditional DNS settings such as Google DNS (8.8.8.8) or Cloudflare DNS (1.1.1.1) may not be sufficient protection when connected directly to a compromised network.
...
Recommended Actions
- Ensure corporate VPN is enabled before accessing company resources on public Wi-Fi networks.
- Be cautious of unexpected Microsoft device code or device pairing prompts.
- Immediately disconnect from the network if any SSL/TLS certificate warnings are displayed when accessing Microsoft 365 services.
- Report any suspicious login prompts or authentication requests to the Information Security team.";

            var emailTitle = "CRITICAL ALERT: Cyber Espionage Campaign (Public Wi-Fi targeting Microsoft 365)";
            byte[] attachmentBytes = null;
            var attachmentPath = @"C:\Users\DELL\.gemini\antigravity\scratch\TealHunt\report templates\Linux 'Dirty Frag' zero-day.docx";
            
            if (System.IO.File.Exists(attachmentPath))
            {
                attachmentBytes = await System.IO.File.ReadAllBytesAsync(attachmentPath);
            }
            
            await emailProvider.SendAsync(null, emailTitle, emailBody, attachmentBytes, "Threat_Advisory_Report.docx");
        }
